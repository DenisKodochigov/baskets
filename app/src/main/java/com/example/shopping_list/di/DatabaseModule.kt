            ...
            .addCallback( object: RoomDatabase.Callback(){
                override fun onCreate(db: SupportSQLiteDatabase) {
                    ...
                    ioThread {
                        ...
                        database.dataDao().newArticle(ArticleEntity(nameArticle = "Молоко", groupId = 1, unitId = 4))
                        database.dataDao().newArticle(ArticleEntity(nameArticle = "Масло", groupId = 1, unitId = 3))
                        database.dataDao().newArticle(ArticleEntity(nameArticle = "Клей", groupId = 1, unitId = 1))
                        database.dataDao().newArticle(ArticleEntity(nameArticle = "Картошка", groupId = 1, unitId = 2))
                        database.dataDao().newProduct(ProductEntity(articleId = 1, value = 1.0, basketId = 1))
                        database.dataDao().newProduct(ProductEntity(articleId = 2, value = 1.0, basketId = 1))
                        database.dataDao().newProduct(ProductEntity(articleId = 3, value = 1.0, basketId = 1))
                        database.dataDao().newProduct(ProductEntity(articleId = 4, value = 1.0, basketId = 1))
                    }
                }
            })
            .build()
