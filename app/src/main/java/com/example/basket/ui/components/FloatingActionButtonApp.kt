...
fun FloatingActionButtonApp(offset: Dp, top: Dp, icon: ImageVector, onClick:() -> Unit){
    FloatingActionButton(
        onClick = onClick, modifier = Modifier.padding(top = top).offset( 0.dp, offset).size(40.dp),)
    {
        Icon( icon, null,
            tint = MaterialTheme.colorScheme.onPrimaryContainer,
            modifier = Modifier.size(28.dp).background(color = MaterialTheme.colorScheme.primaryContainer)) }
}
