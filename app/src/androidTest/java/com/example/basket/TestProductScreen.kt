...
class TestProductScreen {
    ...
    fun setUp() {
        hiltRule.inject()
        composeRule.activity.setContent { MainApp() }
...
//        composeRule.onRoot(useUnmergedTree = true).printToLog(tag = "KDS")
    ...
    fun checkEditBasket(){
        ...
        composeRule.onNodeWithTag(TagsTesting.DIALOG_EDIT_BASKET).assertExists()
        composeRule.onNodeWithTag(TagsTesting.DIALOG_EDIT_BASKET_INPUT_NAME).assertExists()
        composeRule.onNodeWithTag(TagsTesting.DIALOG_EDIT_BASKET_INPUT_NAME).performTextClearance()
        composeRule.onNodeWithTag(TagsTesting.DIALOG_EDIT_BASKET_INPUT_NAME).performTextInput(newBasketName)
        composeRule.onNodeWithTag(TagsTesting.BUTTON_OK, useUnmergedTree = true).performClick()
        composeRule.onNodeWithTag(TagsTesting.BASKET_LAZY).performTouchInput {
            swipeUp(startY = bottom, endY = top , durationMillis = 200) }
        composeRule.waitUntil(timeoutMillis = 4000L, condition = { true })
        composeRule.onNode(basketN).assertIsDisplayed()
